...
app . use ( `/api` , async ( request , response ) =>
	{
		try
		{
			router . get ( `/` , api_cache ( `3 minutes` ) , async ( request , response , next ) =>
				{
					try
					{
						...
						const results = await needle ( `get` , `${ api_key_url }?${ arguments }` ) ;
		...
		catch ( err )
		{
			...
			response . status ( 500 ) . json ( { response : `Error proxy-serving your API key.` } ) ;
			return ;
		}
	}
) ;
...
app . listen ( port , () =>
	{
		console . log ( `api-server listening on port: ` . brightWhite , `${ port }` . brightGreen ) ;
		return ;
	}
) ;
